...
def forward(model, generator, return_input=False, 
    ...
    for n, batch_data_dict in enumerate(generator):
        print(n)
        batch_noise = move_data_to_device(batch_data_dict['noise'], device)
        
        with torch.no_grad():
            model.eval()
            batch_output = model(batch_noise)

        append_to_dict(output_dict, 'signal_trace_name', batch_data_dict['signal_trace_name'])
        ...
        if return_input:
            append_to_dict(output_dict, 'noise', batch_data_dict['noise'])
        ...
        if n % 10 == 0:
            print(' --- Inference time: {:.3f} s / 200 iterations ---'.format(
                time.time() - time1))
            time1 = time.time()
